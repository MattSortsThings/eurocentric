...
namespace Eurocentric.Apis.Admin.ArchitectureTests;

[Category("architecture")]
public sealed class AdminApiArchitectureTests
{
    private static readonly Architecture ArchitectureUnderTest = new ArchLoader()
        .LoadAssembly(typeof(CountryType).Assembly)
        .Build();
    ...
    private static readonly IObjectProvider<Class> FeatureCommandClasses = Classes()
        .That().AreNestedIn(FeatureClasses)
        .And().HaveName("Command");

    private static readonly IObjectProvider<Class> FeatureQueryClasses = Classes()
        .That().AreNestedIn(FeatureClasses)
        .And().HaveName("Query");

    private static readonly IObjectProvider<Class> FeatureCommandHandlerClasses = Classes()
        .That().AreNestedIn(FeatureClasses)
        .And().HaveName("CommandHandler");
    ...
    [Test]
    public async Task Classes_that_implement_ICommand_should_be_feature_command_or_request_classes()
    {
        // Arrange
        ClassRule rule = Classes()
            .That().ImplementInterface(typeof(ICommand<>))
            .Should().Be(FeatureCommandClasses)
    ...
    [Test]
    [Skip("No feature command classes yet")]
    public async Task Feature_command_classes_should_be_internal()
    {
        // Arrange
        ClassRule rule = Classes()
            .That().Are(FeatureCommandClasses)
            .Should().BeInternal();
    ...
    [Test]
    [Skip("No feature command classes yet")]
    public async Task Feature_command_classes_should_be_immutable_records()
    {
        // Arrange
        ClassRule rule = Classes()
            .That().Are(FeatureCommandClasses)
    ...
    [Test]
    [Skip("No feature command classes yet")]
    public async Task Feature_command_classes_should_not_depend_on_any_domain_assembly_types()
    {
        // Arrange
        ClassRule rule = Classes()
            .That().Are(FeatureCommandClasses)
    ...
    [Test]
    public async Task Classes_that_implement_ICommandHandler_should_be_feature_command_handler_classes()
    {
        // Arrange
        ClassRule rule = Classes()
            .That().ImplementInterface(typeof(ICommandHandler<,>))
            .Should().Be(FeatureCommandHandlerClasses);
