    ...
    private sealed class AdminActor : ActorWithResponse<GetBroadcastResponse>
    {
            ...
            string contestStage = "")
        {
            ...
            List<DomainCompetitor> competitors = competingCountryCodes?.Select(code => MyCountryCodesAndIds[code])
                ...
                .ToList() ?? [];
            List<DomainJury> juries = MyCountryCodesAndIds.Values.Select(id => CountryId.FromValue(id))
                .Select(id => new DomainJury(id))
                .ToList();
            List<DomainTelevote> televotes = MyCountryCodesAndIds.Values.Select(id => CountryId.FromValue(id))
