...
public sealed class HandleBroadcastDeletedTests : SerialCleanAcceptanceTest
{
    ...
    public async Task DomainEventHandler_should_update_parent_contest_when_broadcast_deleted_scenario_1_of_3(string apiVersion)
    {
    ...
    [Test]
    [ApiVersion1Point0AndUp]
    public async Task DomainEventHandler_should_update_parent_contest_when_broadcast_deleted_scenario_2_of_3(string apiVersion)
    {
        AdminActor admin = new(ApiDriver.Create(SystemUnderTest, apiVersion));
    ...
    [Test]
    [ApiVersion1Point0AndUp]
    public async Task DomainEventHandler_should_update_parent_contest_when_broadcast_deleted_scenario_3_of_3(string apiVersion)
    {
        AdminActor admin = new(ApiDriver.Create(SystemUnderTest, apiVersion));
    ...
    private sealed class AdminActor(IApiDriver apiDriver) : AdminActorWithoutResponse(apiDriver)
    {
        ...
        public async Task Then_my_contest_should_now_match(string? childBroadcasts = null,
            bool completed = true)
        {
            Guid myContestId = await Assert.That(ContestId).IsNotNull();
