...
{
    public sealed partial class Endpoint : AcceptanceTest
    {
        [Theory]
        [InlineData("v1.0")]
        public async Task Should_create_and_return_Stockholm_format_contest_scenario_1(string apiVersion)
        {
            ...
            admin.Then_the_created_contest_should_match(contestFormat: "Stockholm",
                contestYear: 2016,
                cityName: "Stockholm",
                completed: false,
                childBroadcasts: 0,
        ...
        [Theory]
        [InlineData("v1.0")]
        public async Task Should_fail_on_Stockholm_format_contest_with_duplicate_group_1_and_2_participating_country_IDs(
            string apiVersion)
        {
            Admin admin = new(RestClient, BackDoor, RequestFactory.WithApiVersion(apiVersion));
