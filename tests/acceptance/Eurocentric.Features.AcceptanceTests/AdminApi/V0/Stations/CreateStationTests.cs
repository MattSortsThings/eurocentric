...
public sealed class CreateStationTests(WebAppFixture webAppFixture) : AcceptanceTestBase(webAppFixture)
{
    ...
    public async Task AdminApi_should_return_Created_with_created_station_given_valid_request()
    {
        ...
        RestRequest restRequest = Post(Route).UseSecretApiKey().AddJsonBody(request);
    ...
    {
        // Arrange
        RestRequest restRequest = Post(Route).UseSecretApiKey().AddJsonBody(new { Name = "Name", Line = 999999 });
