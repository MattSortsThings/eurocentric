...
public abstract class Actor
{
    ...
    public async Task Then_my_request_should_FAIL_with_status_code_409_Conflict() => await Assert.That(ResponseStatusCode)
        .IsEqualTo(HttpStatusCode.Conflict);

    public async Task Then_my_request_should_FAIL_with_status_code_422_UnprocessableEntity() => await Assert
        .That(ResponseStatusCode)
