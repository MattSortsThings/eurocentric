...
public sealed class GetCompetitorPointsShareRankingsTests : ParallelSeededAcceptanceTest
{
    ...
    public async Task Endpoint_should_fail_on_invalid_minYear_and_maxYear_query_params(string apiVersion)
    {
        ...
        // Given
        euroFan.Given_I_want_to_obtain_a_page_of_competitor_points_share_rankings(minYear: 2050, maxYear: 2016);
        ...
        : EuroFanActorWithResponse<GetCompetitorPointsShareRankingsResponse>(apiDriver)
    {
            ...
            string? contestStage = null)
        {
            ...
            Request = ApiDriver.RequestFactory.Rankings.GetCompetitorPointsShareRankings(queryParams);
        }
        ...
        public async Task Then_the_response_filtering_metadata_should_match(string votingMethod = "",
            ...
            string contestStage = "")
        {
            (_, CompetitorPointsShareFilteringMetadata filtering, _) = await Assert.That(ResponseBody).IsNotNull();

            CompetitorPointsShareFilteringMetadata expectedFiltering = new()
            {
