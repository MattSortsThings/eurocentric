...
public sealed class GetCompetitorPointsInRangeRankingsTests : ParallelSeededAcceptanceTest
{
        ...
        : EuroFanActorWithResponse<GetCompetitorPointsInRangeRankingsResponse>(apiDriver)
    {
            ...
            int? maxPoints = null)
        {
            ...
            Request = ApiDriver.RequestFactory.CompetitorRankings.GetCompetitorPointsInRangeRankings(queryParams);
        }
        ...
        public async Task Then_the_response_query_metadata_should_match(string votingMethod = "",
            ...
            int maxPoints = 0)
        {
            (_, CompetitorPointsInRangeQueryMetadata query, _) = await Assert.That(ResponseBody).IsNotNull();

            CompetitorPointsInRangeQueryMetadata expectedQuery = new()
            {
