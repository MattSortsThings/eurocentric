...
public abstract class ActorWithResponse<T>
    where T : class
{
    protected ActorWithResponse(IActorKernel kernel)
    {
        Kernel = kernel;
    }
    ...
    private protected RestResponse<T>? SuccessResponse { get; private set; }
    ...
    public async Task When_I_send_my_request()
    {
        ...
        UnionRestResponse<T> response = await Kernel.RestClient.SendRequestAsync<T>(request);
