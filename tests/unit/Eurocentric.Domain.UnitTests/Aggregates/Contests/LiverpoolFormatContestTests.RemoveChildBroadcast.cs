...
{
    [Test]
    public async Task CompleteChildBroadcast_should_remove_ChildBroadcast()
    {
        // Arrange
        ...
        // Assert
        await Assert.That(sut.ChildBroadcasts).HasCount(2);

        // Act
        sut.RemoveChildBroadcast(broadcastId1Of2);

        // Assert
        await Assert.That(sut.ChildBroadcasts).HasSingleItem()
            .And.ContainsOnly(Matchers.ChildBroadcastNotCompleted(broadcastId2Of2, ContestStage.SemiFinal2));
    }

    [Test]
    public async Task CompleteChildBroadcast_should_set_Completed_to_false()
    {
        // Arrange
        LiverpoolFormatContest sut = CreateContest(group0CountryId: XxId,
            group1CountryIds: [AtId, BeId, CzId],
        ...
        // Act
        sut.RemoveChildBroadcast(broadcastId1Of3);
