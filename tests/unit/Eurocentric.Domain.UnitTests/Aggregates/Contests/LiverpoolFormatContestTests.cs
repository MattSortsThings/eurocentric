...
public sealed class LiverpoolFormatContestTests : UnitTest
{
        ...
        string contestIdValue)
    {
        ...
        // Act
        ErrorOr<Contest> result = LiverpoolFormatContest.Create()
        ...
        LiverpoolFormatContest contest = await Assert.That(result.Value).IsTypeOf<LiverpoolFormatContest>()
            .And.IsNotNull();
        ...
        await Assert.That(contest.ContestFormat).IsEqualTo(ContestFormat.Liverpool);
    }
    ...
    public async Task FluentBuilder_should_return_Contest_with_specified_Participants()
    {
        ...
        await Assert.That(contest.Participants)
            .HasCount(7)
            .And.Contains(Matchers.Group0Participant(XxId))
    ...
    [Test]
    public async Task FluentBuilder_should_return_Errors_when_CityName_not_set()
    {
        // Arrange
        Func<ContestId> dummyIdProvider = () => FixedContestId;
        ...
        await Assert.That(result.FirstError).HasType(ErrorType.Unexpected)
            .And.HasCode("City name not set")
            .And.HasDescription("Contest builder invoked without setting city name.");
    }
    ...
    [Test]
    public async Task FluentBuilder_should_return_errors_given_duplicate_group_0_and_1_participating_country_IDs()
    {
        // Arrange
        Func<ContestId> dummyIdProvider = () => FixedContestId;
    ...
    [Test]
    public async Task FluentBuilder_should_return_errors_given_duplicate_group_0_and_2_participating_country_IDs()
    {
        // Arrange
        Func<ContestId> dummyIdProvider = () => FixedContestId;
    ...
    [Test]
    public async Task FluentBuilder_should_return_errors_given_no_group_0_participants()
    {
        // Arrange
        Func<ContestId> dummyIdProvider = () => FixedContestId;
        ...
        await Assert.That(result.FirstError).HasType(ErrorType.Failure)
            .And.HasCode("Illegal Liverpool format participant groups")
            .And.HasDescription("A Liverpool format contest must have one group 0 participant, " +
    ...
    {
        // Arrange
        Action act = () => LiverpoolFormatContest.Create().AddGroup0Participant(null!);
    ...
    {
        // Arrange
        Action act = () => LiverpoolFormatContest.Create().AddGroup1Participant(null!, ArbitraryActName, ArbitrarySongTitle);
    ...
    {
        // Arrange
        Action act = () => LiverpoolFormatContest.Create().AddGroup2Participant(null!, ArbitraryActName, ArbitrarySongTitle);
    ...
    {
        // Arrange
        Action act = () => LiverpoolFormatContest.Create().Build(null!);
