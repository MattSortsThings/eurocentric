...
{
    /// <summary>
    ///     Dispatches the query to the bus and returns the errors or response mapped to <i>EITHER</i> a
    ...
    /// <summary>
    ///     Dispatches the command to the bus and returns the errors or response mapped to <i>EITHER</i> a
    ///     <see cref="ProblemHttpResult" /> (when unsuccessful) <i>OR</i> a successful HTTP result.
    /// </summary>
    /// <param name="bus">The request response bus.</param>
    ...
    internal static async Task<IResult> SendWithResponseMapperAsync<TResponse>(this IRequestResponseBus bus,
        ...
        CancellationToken cancellationToken = default)
    {
        ErrorOr<TResponse> errorsOrResponse = await bus.Send(command, cancellationToken: cancellationToken)
