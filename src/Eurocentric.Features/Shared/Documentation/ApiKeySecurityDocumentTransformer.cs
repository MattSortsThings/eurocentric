    ...
    : IOpenApiDocumentTransformer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (authenticationSchemes.Any(authScheme => authScheme.Name == ApiKeyConstants.SchemeName))
        {
            Dictionary<string, OpenApiSecurityScheme> requirements = new()
            {
                [ApiKeyConstants.SchemeName] = new OpenApiSecurityScheme
                {
                    Type = SecuritySchemeType.ApiKey,
                    Scheme = ApiKeyConstants.SchemeName,
                    In = ParameterLocation.Header,
                    BearerFormat = ApiKeyConstants.BearerFormat,
                    Name = ApiKeyConstants.HttpRequestHeaderName
                }
            };
