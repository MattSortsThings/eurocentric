...
internal sealed class ConfigureScalarOptions : IConfigureOptions<ScalarOptions>
{
    public void Configure(ScalarOptions options)
    {
        ...
        options.AddPreferredSecuritySchemes(ApiKeyConstants.SchemeName);
        options.AddApiKeyAuthentication(ApiKeyConstants.SchemeName, scheme =>
            scheme.Name = ApiKeyConstants.HttpRequestHeaderName);
    }
}
