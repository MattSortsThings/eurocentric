...
internal sealed class TagsDocumentTransformer : BaseTagsDocumentTransformer
{
    ...
    {
        yield return new TagDatum(EndpointNames.Tags.Broadcasts,
            """
            Endpoints using the **Broadcast** resource.

            A Broadcast resource represents a single stage of a single contest. Use the Broadcast endpoints to award points from
            the juries and televotes to the competitors in a broadcast that exists in the system.
            """);
        yield return new TagDatum(EndpointNames.Tags.Contests,
            """
            ...
            """);
        yield return new TagDatum(EndpointNames.Tags.Countries,
            """
