...
internal static class AwardBroadcastJuryPoints
{
    private static async Task<IResult> ExecuteAsync(
        [FromRoute(Name = "broadcastId")] Guid broadcastId,
        [FromBody] AwardBroadcastJuryPointsRequest request,
        [FromServices] IRequestResponseBus bus,
        CancellationToken ct = default
    ) => await bus.DispatchAsync(MapToUnitCommand(broadcastId, request), ct);

    private static UnitCommand MapToUnitCommand(Guid broadcastId, AwardBroadcastJuryPointsRequest request)
    {
