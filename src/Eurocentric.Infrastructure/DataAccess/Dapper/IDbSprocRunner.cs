...
/// </summary>
public interface IDbSprocRunner
{
    /// <summary>
    ///     Asynchronously executes the specified stored procedure with the provided parameters and returns the results.
    /// </summary>
    /// <param name="sprocName">The schema-qualified name of the stored procedure to be run.</param>
    ...
    /// </returns>
    Task<T[]> ExecuteAsync<T>(string sprocName, DynamicParameters parameters, CancellationToken cancellationToken = default)
        where T : class;
}
