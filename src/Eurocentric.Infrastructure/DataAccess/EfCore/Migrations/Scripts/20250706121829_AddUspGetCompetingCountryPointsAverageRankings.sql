...
BEGIN
    ...
    WITH data AS
             (SELECT t.competing_country_code,
              ...
              WHERE (@include_televote_awards = 1)
                AND (t.contest_year BETWEEN @min_contest_year AND @max_contest_year)
         ...
         agg AS
             (SELECT data.competing_country_code              AS country_code,
                     AVG(data.real_points_value)              AS points_average,
    ...
    SELECT DENSE_RANK() OVER (ORDER BY agg.points_average DESC) AS rank,
           agg.country_code,
           agg.points_average,
           agg.points_awards,
           agg.broadcasts,
