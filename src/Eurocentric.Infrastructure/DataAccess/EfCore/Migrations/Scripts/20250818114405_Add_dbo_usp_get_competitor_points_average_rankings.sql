/*
    Creates the stored procedure dbo.usp_get_competitor_points_average_rankings.
*/

CREATE PROCEDURE dbo.usp_get_competitor_points_average_rankings(
    @exclude_jury BIT = 0,
...
BEGIN
    ...
    WITH
        ...
        t3 AS (SELECT t2.competing_country_id,
                      t2.broadcast_id,
                      t2.contest_id,
                      CAST(t2.total_points AS FLOAT) / t2.points_awards AS points_average,
                      t2.total_points,
                      t2.points_awards
               FROM t2)
