...
internal sealed class ContestConfig : IEntityTypeConfiguration<Contest>
{
    ...
    private static void AddContestFormatEnumCheckConstraint(TableBuilder<Contest> builder) =>
        builder.HasCheckConstraint("ck_contest_contest_format_enum_value",
            "[contest_format] BETWEEN 0 AND 1");
    ...
    private static void AddContestStageEnumCheckConstraint(OwnedNavigationTableBuilder<Contest, ChildBroadcast> builder) =>
        builder.HasCheckConstraint("ck_contest_child_broadcast_contest_stage_enum_value",
    ...
    private static void ConfigureAsTable(OwnedNavigationBuilder<Contest, Participant> builder)
    {
        ...
        builder.ToTable(AddPermittedNullabilityCheckConstraint);
    }

    private static void AddParticipantGroupEnumCheckConstraint(OwnedNavigationTableBuilder<Contest, Participant> builder) =>
        builder.HasCheckConstraint("ck_contest_participant_participant_group_enum_value",
            "[participant_group] BETWEEN 0 AND 2");

    private static void AddPermittedNullabilityCheckConstraint(OwnedNavigationTableBuilder<Contest, Participant> builder) =>
        builder.HasCheckConstraint("ck_contest_participant_permitted_nullability",
