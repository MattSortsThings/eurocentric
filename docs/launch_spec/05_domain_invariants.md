...
- A `Country` with a non-empty active `ContestId` collection cannot be deleted
...
- Every participating `Country` in the created `Contest` must add its ID to its active `ContestId` collection
- The global televote voting `Country` in the created `Contest`, if present, must add its ID to its active `ContestId` collection

### Delete contest

- A `Contest` with one or more child broadcasts cannot be deleted
- Every participating `Country` in the deleted `Contest` must remove its ID from its active `ContestId` collection
- The global televote voting `Country` in the deleted `Contest`, if present, must remove its ID from its active `ContestId` collection
...
- There must be at least 1 performing spot in each half
- The parent `Contest` of the created `Broadcast` must add a corresponding child broadcast

### Delete broadcast

- The parent `Contest` of the deleted `Broadcast` must remove the corresponding child broadcast and set itself to `Queryable=false` if it was previously `Queryable=true`
...
- If the `Broadcast` is set to `Completed=true`, the parent `Contest` of the completed `Broadcast` must replace the corresponding child broadcast and set itself to `Queryable=true` if it now has 3 child broadcasts, all `Completed=true`
