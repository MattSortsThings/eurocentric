...
- A simple logging system is defined, which writes each log entry synchronously to the system database; this is not optimal, but is good enough for the project
...
|    *logging*     | Simple API request/response logging      |
...
|             Feature             | Reason                                                                                                                                                                              |
|:-------------------------------:|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Database concurrency management | Only a single user (the Admin) will ever INSERT/UPDATE/DELETE records in the domain aggregate tables. Only a single process will ever INSERT/DELETE records in the log entry table. |
