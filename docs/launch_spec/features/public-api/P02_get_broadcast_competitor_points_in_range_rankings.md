...
- **As a EuroFan**
- **I want** to rank individual broadcast competitors by their *points in range* metric, i.e. the relative frequency of points awards the country the competitor received in their broadcast having a value within a specific range, receiving the query metadata and a page of rankings ordered by rank, then by contest stage (`SemiFinal1` &lt; `SemiFinal2` &lt; `GrandFinal`), then by performing spot
...
```http request
GET /public/api/{apiVersion}/rankings/broadcast-competitors/points-in-range
```
