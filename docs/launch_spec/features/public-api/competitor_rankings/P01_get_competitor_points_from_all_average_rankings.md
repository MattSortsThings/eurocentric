# P01. Get competitor points from all average rankings
...
- **As a EuroFan**
- **I want** to rank all broadcast competitors by their *points from all average* metric
  - **i.e.** the average individual points award value each competitor received from all voting countries in their broadcast
...
5. Rank all competitors by descending points from all average (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/competitor-rankings/points-from-all-average
```
