# P03. Get competitor points from all in range rankings
...
- **As a EuroFan**
- **I want** to rank all broadcast competitors by their *points from all in range* metric
  - **i.e.** the relative frequency of points awards within a specified value range each competitor received from all voting countries in their broadcast
...
5. Rank all competitors by descending points from all in range (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/competitor-rankings/points-from-all-in-range
```
