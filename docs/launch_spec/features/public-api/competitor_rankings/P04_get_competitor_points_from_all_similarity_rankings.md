# P04. Get competitor points from all similarity rankings
...
- **As a EuroFan**
- **I want** to rank all broadcast competitors by their *points from all similarity* metric
  - **i.e.** the cosine similarity between the normalized jury and televote points award values each competitor received from all voting countries in their broadcast
...
4. Group the filtered data by broadcast competitor
5. For each group, calculate:
   - the jury and televote vectors, using each broadcast voter as a dimension
   ...
   - the points from all similarity (rounding half-up to 6dp)
6. Rank all competitors by descending points from all similarity (using non-dense ranking, assigning equal rank to equal values)
7. Sort the rankings by *either* ascending *or* descending (rank, broadcast date, performing spot) 3-tuple
8. Apply pagination
...
```http request
GET /public/api/{apiVersion}/competitor-rankings/points-from-all-similarity
```
  ...
  "rankings": [
    {
      ...
      "finishingSpot": 1,
      "vectorDimensions": 50,
      "vectorDotProduct": 1000.0,
      "juryVectorMagnitude": 40.0,
      "televoteVectorMagnitude": 50.0,
      "pointsFromAllSimilarity": 0.5
    }
  ]
}
```
