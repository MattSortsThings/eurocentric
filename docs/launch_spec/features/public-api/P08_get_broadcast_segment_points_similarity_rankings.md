# P08 Get broadcast segment points similarity rankings
...
- **As a EuroFan**
- **I want** to rank broadcast running order segments by their *points similarity* metric, i.e. the cosine similarity between the normalized televote and jury points awards received by competitors performing in the segment across broadcasts, receiving the query metadata and the rankings ordered by rank, then by segment
...
```http request
GET /public/api/{apiVersion}/rankings/broadcast-segments/points-similarity
```
...
**GetBroadcastSegmentPointsSimilarityRankings endpoint...**
