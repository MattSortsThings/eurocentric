# P05 Get broadcast segment points average rankings
...
- **As a EuroFan**
- **I want** to rank broadcast running order segments by their *points average* metric, i.e. the average points award value received by competitors performing in the segment across broadcasts, receiving the query metadata and the rankings ordered by rank, then by segment
...
```http request
GET /public/api/{apiVersion}/rankings/broadcast-segments/points-average
```
...
- `pointsAverage` is a decimal value, calculated as `totalPoints` / `pointsAwards`, rounded to 6 decimal places

## Acceptance criteria

### Happy path

**GetBroadcastSegmentPointsAverageRankings endpoint...**
