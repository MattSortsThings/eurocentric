# P15. Get country points to single fraction rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points to single fraction* metric
  - **i.e.** the fraction of the maximum possible points each country gave to a specified competing country across broadcasts
...
4. For each group, calculate:
   ...
   - the points to single fraction (rounding half-up to 6dp)
5. Rank all voting countries by descending points to single fraction (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-to-single-fraction
```
