# P06. Get country points from all fraction rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points from all fraction* metric
  - **i.e.** the fraction of the maximum possible points each country received from all voting countries across broadcasts
...
5. Rank all competing countries by descending points from all fraction (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-from-all-fraction
```
