# P13. Get country points to all similarity rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points to all similarity* metric
  - **i.e.** the cosine similarity between the normalized jury and televote points award values each country gave to all competing countries across broadcasts
...
5. For each group, calculate:
   - the jury and televote vectors, using each broadcast competitor as a dimension
   - the count of unique contests
   - the count of unique broadcasts
   - the count of unique competing countries
   ...
   - the points to all similarity (rounding half-up to 6dp)
6. Rank all voting countries by descending points to all similarity (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-to-all-similarity
```
  ...
  "rankings": [
    {
      ...
      "contests": 5,
      "broadcasts": 10,
      "competingCountries": 50,
      "vectorDimensions": 500,
      "vectorDotProduct": 10000.0,
      "juryVectorMagnitude": 100.0,
      "televoteVectorMagnitude": 200.0,
      "pointsToAllSimilarity": 0.5
    }
  ]
}
```
