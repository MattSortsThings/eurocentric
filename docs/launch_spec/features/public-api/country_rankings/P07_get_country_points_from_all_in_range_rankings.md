# P07. Get country points from all in range rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points from all in range* metric
  - **i.e.** the relative frequency of points awards within a specified value range each country received from all voting countries across broadcasts
...
4. For each group, calculate:
   ...
   - the count of unique voting countries
   - the count of points awards
   - the count of points awards in range
   - the points from all in range (rounding half-up to 6dp)
5. Rank all competing countries by descending points from all in range (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-from-all-in-range
```
  ...
  "rankings": [
    {
      ...
      "votingCountries": 50,
      "pointsAwards": 1000,
      "pointsAwardsInRange": 750,
      "pointsFromAllInRange": 0.75
    }
  ]
}
```
...
- [ ] Succeeds_when_specifying_minPointsValue_1_maxPointsValue_8
- [ ] Succeeds_when_specifying_minPointsValue_1_maxPointsValue_12
- [ ] Succeeds_when_specifying_minPointsValue_8_maxPointsValue_12
...
- [ ] Fails_when_minPointsVaue_is_greater_than_maxPointsValue
