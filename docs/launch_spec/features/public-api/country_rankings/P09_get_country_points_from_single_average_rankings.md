# P09. Get country points from single average rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points from single average* metric
  - **i.e.** the average individual points award value each country received from a specified voting country across broadcasts
...
4. For each group, calculate:
   ...
   - the points from single average (rounding half-up to 6dp)
5. Rank all competing countries by descending points from single average (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-from-single-average
```
