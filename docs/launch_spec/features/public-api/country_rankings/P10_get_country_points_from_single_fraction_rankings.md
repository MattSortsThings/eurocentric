# P10. Get country points from single fraction rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points from single fraction* metric
  - **i.e.** the fraction of the maximum possible points each country received from a specified voting country across broadcasts
...
4. For each group, calculate:
   ...
   - the points from single fraction (rounding half-up to 6dp)
5. Rank all competing countries by descending points from single fraction (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-from-single-fraction
```
...
- [ ] Succeeds_when_specifying_votingCountryCode_XX
