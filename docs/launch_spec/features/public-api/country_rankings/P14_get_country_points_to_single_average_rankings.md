# P14. Get country points to single average rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points to single average* metric
  - **i.e.** the average individual points award value each country gave to a specified competing country across broadcasts
...
4. For each group, calculate:
   ...
   - the points to single average (rounding half-up to 6dp)
5. Rank all voting countries by descending points to single average (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-to-single-average
```
