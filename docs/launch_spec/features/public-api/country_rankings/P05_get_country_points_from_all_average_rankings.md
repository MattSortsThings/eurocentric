# P05. Get country points from all average rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points from all average* metric
  - **i.e.** the average individual points award value each country received from all voting countries across broadcasts
...
4. For each group, calculate:
   ...
   - the points from all average (rounding half-up to 6dp)
5. Rank all competing countries by descending points from all average (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-from-all-average
```
