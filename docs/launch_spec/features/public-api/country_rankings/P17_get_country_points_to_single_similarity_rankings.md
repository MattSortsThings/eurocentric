# P17. Get country points to single similarity rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points to single similarity* metric
  - **i.e.** the cosine similarity between the normalized jury and televote points award values each country gave to a specified competing country across broadcasts
...
4. Group the filtered data by voting country
5. For each group, calculate:
   ...
   - the points to single similarity (rounding half-up to 6dp)
6. Rank all voting countries by descending points to single similarity (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-to-single-similarity
```
...
- [ ] Succeeds_when_specifying_competingCountryCode_AT
- [ ] Succeeds_when_specifying_competingCountryCode_FI
- [ ] Succeeds_when_specifying_competingCountryCode_GB
- [ ] Succeeds_when_specifying_competingCountryCode_SM
...
- [ ] Succeeds_with_0_rankings_when_competingCountryCode_matches_no_voters
