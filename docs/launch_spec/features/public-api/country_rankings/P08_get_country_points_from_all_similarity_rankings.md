# P08. Get country points from all similarity rankings
...
- **As a EuroFan**
- **I want** to rank all countries by their *points from all similarity* metric
  - **i.e.** the cosine similarity between the normalized jury and televote points award values each country received from all voting countries across broadcasts
...
6. Rank all competing countries by descending points from all similarity (using non-dense ranking, assigning equal rank to equal values)
...
```http request
GET /public/api/{apiVersion}/country-rankings/points-from-all-similarity
```
