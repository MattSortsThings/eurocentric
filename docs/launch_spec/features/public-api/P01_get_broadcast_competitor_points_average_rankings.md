...
- **As a EuroFan**
- **I want** to rank individual broadcast competitors by their *points average* metric, i.e. the average points award value the competitor received in their broadcast, receiving the query metadata and a page of rankings ordered by rank, then by contest stage (`SemiFinal1` &lt; `SemiFinal2` &lt; `GrandFinal`), then by performing spot
...
```http request
GET /public/api/{apiVersion}/rankings/broadcast-competitors/points-average
```
...
**GetBroadcastCompetitorPointsAverageRankings endpoint...**
