...
```http request
PATCH /admin/api/{apiVersion}/broadcasts/{broadcastId}/televote-points
```
...
- `votingCountryId` must reference a televote in the broadcast that is `PointsAwarded=false`
- `rankedCompetingCountryIds` must be equivalent to the set of all competing country IDs in the broadcast excluding the voting country ID of the televote
...
- Should_set_televote_to_PointsAwarded_after_awarding_points
- Should_set_broadcast_to_Completed_when_final_televote_and_jury_is_set_to_PointsAwarded
- Should_set_TelevoteOnly_broadcast_to_Completed_when_final_televote_is_set_to_PointsAwarded
...
- Should_fail_when_televote_is_PointsAwarded
