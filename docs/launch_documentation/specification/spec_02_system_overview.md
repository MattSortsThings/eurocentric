...
The system is a web API for modelling and (over)analysing the Eurovision Song Contest, 2016-2025.
...
- Ranking competing countries by metrics calculated from points received across broadcasts.
- Ranking competitors in broadcasts by metrics calculated from points received in their broadcasts.
- Ranking voting countries by metrics calculated from points given across broadcasts.
- Retrieving lists of queryable countries, contests and broadcasts.
- Retrieving the scoreboard from a given broadcast.
...
- Creating, retrieving, modifying and deleting broadcasts in the system.
...
- A single software developer (the author).
- The designer of the system, including its domain model and database schema.
- Located in the UK.
- Highly familiar with the structure and recent history of the Eurovision Song Contest.
- Knowledgeable about how the voting system works.
- Able to send HTTP requests and read responses.
