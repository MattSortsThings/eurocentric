...
- The parent `Contest` of the deleted `Broadcast` must remove the corresponding broadcast memo and set itself to `Completed=false` if it was previously `Completed=true`.
...
- If the `Broadcast` is set to `Completed=true`, the parent `Contest` of the completed `Broadcast` must replace the corresponding broadcast memo and set itself to `Completed=true` if it now has 3 broadcast memos, all `Completed=true`.
