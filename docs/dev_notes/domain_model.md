...
2. A **BROADCAST** has a unique (contest ID, contest stage) tuple.
3. A **BROADCAST** has a status, which is one of { `Initialized`, `InProgress`, `Completed` }.
4. A **BROADCAST**'s status is `Completed` when all of its **Juries** and all of its **Televotes** have awarded their points.
5. A **BROADCAST** has at least 2 **Competitors**.
6. A **BROADCAST** has zero or multiple **Juries**.
7. A **BROADCAST** has at least 2 **Televotes**.
8. A **Jury** awards its points by ordering all the **Competitors** from first to last, excluding the **Competitor** representing the **Jury**'s **COUNTRY** if present.
9. A **Televote** awards its points by ordering all the **Competitors** from first to last, excluding the **Competitor** representing the **Televote**'s **COUNTRY** if present.
10. A **Competitor** may be disqualified from a **BROADCAST** only if its status is `Initialized`.
11. When a **Competitor** is disqualified from a **BROADCAST**, the remaining **Competitors** update their finishing positions to close the gap but leave their running order positions unchanged.
