...
```
PATCH /admin/api/v1.0/broadcasts/{broadcastId}/award-jury
```
...
```
PATCH /admin/api/v1.0/broadcasts/{broadcastId}/award-televote
```
...
```
PATCH /admin/api/v1.0/broadcasts/{broadcastId}/disqualify
```
...
- **SO THAT** the *Public API* has an up-to-date list of queryable countries when I refresh the queryable data.

## Queryable Data

### A21: Refresh Queryable Data

**User Story:**

- **AS** the *Admin*
  - who has created and completed one or more contests in the system
- **I WANT** to refresh the queryable data based on the completed contests
- **SO THAT** the *Public API*'s queryable data is up-to-date.

**Endpoint:**

```
POST /admin/api/v1.0/queryable-data/refresh
```
