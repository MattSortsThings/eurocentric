...
- remove a **ChildBroadcast**, which may also update its `Completed` value.
- complete a **ChildBroadcast**, which may also update its `Completed` value.
...
8. A **CONTEST** is `Completed=true` only if it has 3 **ChildBroadcasts** and they are all `Completed=true`.
