...
The Public API's queryable voting data is a subset of the data created and updated by the Admin API. Specifically, the queryable voting data is defined as follows:

- A contest is queryable if all three of its child broadcasts have been created and all their points are awarded.
- A broadcast is queryable if its parent contest is queryable.
- A country is queryable if it has a role in one or more queryable contests.
...
- The web application is published as a Docker image on Docker Hub.
- The containerized web application is hosted in the cloud using the Azure Container Apps service.
