...
The queryable data is kept up to date using **database triggers**, which run every time a **CONTEST** aggregate is updated or deleted.
...
The *Public API* applies a multitude of read-only queries to a set of queryable data. The queryable data is derived from the domain model data, which in turn is created, updated, and deleted using the *Admin API*.

The two APIs' data is kept in sync using a `QueryableDataRefreshed` integration event, which is triggered manually by the *Admin*.
...
- The *public-api* filters and scoreboards features interact with the database using the same **EF Core** database context encapsulated behind a read-only repository service.
- The *public-api* rankings features interact with the database using stored procedures, invoked using **Dapper**, encapsulated behind a read-only paginated repository service.
