...
|   `PATCH`   |         200          | Summary of modified resource   | None              |
...
|         500          | "An unexpected exception was thrown, which is not a `BadHttpRequestException` or an `InvalidEnumArgumentException`."                                                                                                                                          | Database connection times out.                                                                                                                      |
...
Features with *admin-api* scope are tested using a web application fixture that is reset to an empty state after every test. This means that all *admin-api* acceptance tests are run sequentially. Features are tested through user specifications written in a [Gherkin](https://cucumber.io/docs/gherkin/)-like syntax.

Features with *public-api* scope are tested using a web application feature that is seeded once at the start of the test run. This means that all *public-api* acceptance tests are run concurrently. This is possible because the *Public API* only has GET endpoints. Features are tested through user specifications written in a [Gherkin](https://cucumber.io/docs/gherkin/)-like syntax.

Features with *shared* scope are tested using a web application fixture that is reset to an empty state after every test. This means that all *admin-api* acceptance tests are run sequentially. Features are tested using explicit HTTP requests and responses.
