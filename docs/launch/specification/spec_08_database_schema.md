...
- In the `broadcast` table:
  ...
  - `contest_stage` is one of: `SemiFinal1`, `SemiFinal2`, `GrandFinal`.
...
- In the `contest` table:
  - `contest_rules` is one of: `Liverpool`, `Stockholm`.
...
- In the `child_broadcast` table:
  ...
  - `contest_stage` is one of: `SemiFinal1`, `SemiFinal2`, `GrandFinal`.
- In the `competitor` table:
  - `semi_final_draw` is one of: `SemiFinal1`, `SemiFinal2`.
...
- In the `contest_role` table:
  ...
  - `contest_role_type` is one of: `Participant`, `GlobalTelevote`.
