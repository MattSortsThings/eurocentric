...
- [6. Domain model](#6-domain-model)
  ...
  - [Compound value objects](#compound-value-objects)
    ...
    - [*Pagination* value object](#pagination-value-object)
...
A *ContestRoleType* value specifies a country's role in a contest.
...
A *PointsValue* is an `integer` that specifies the numeric value of a points award in a broadcast.

**Invariants:**

1. A *PointsValue* value is not negative.
...
A *Pagination* is an (`integer PageIndex`, `integer PageSize`, `bool descending`) tuple that sets the pagination settings for a query on the voting data.

**Invariants:**

1. A *Pagination* `PageIndex` value is not negative.
2. A *Pagination* `PageSize` value is not less than 1.

### *ContestYearRange* value object

A *ContestYearRange* is a (`ContestYear?, ContestYear?)` tuple that represents the optional inclusive contest year range for a query;

**Invariants:**

1. The maximum *ContestYear* must not be less than the minimum *ContestYear* when both are not `null`.

### *PointsValueRange* value object

A *PointsValueRange* is a (*PointsValue*, *PointsValue*) tuple that represents the inclusive points value range for a query, where the first item is the minimum and the second item is the maximum.

**Invariants:**

1. The maximum *PointsValue* must not be less than the minimum *PointsValue*.
